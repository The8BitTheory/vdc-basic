...
182 FOR FI=1TO19
183 VMF AP,0,8000: REM CLEAR ATTRIBUTE RAM
...
615 PO=DB+6
#   METADATA-LENGTH:11 BYTES (6 BYTES HEADER, 5 BYTES COMMAND DATA)
616 POKE DB,11:POKE DB+1,0
#   IMAGEDATA-LENGTH
617 POKE DB+2,FNLB(FL):POKE DB+3,FNHB(FL)
#   ALPHA-COLOR. VALUE LARGER THAN 15 MEANS: NO ALPHA
618 POKE DB+4,255
#   COMMAND COUNT. 1. JUST A RECTANGULAR VMC
619 POKE DB+5,1

# OFFSETS
# 0:COMMAND-ID 2
# 1,2:INITIAL TARGET ADDRESS OFFSET
# 3:BYTE COUNT FOR COPY (WIDTH)
# 4:NR REPETITIONS (HEIGHT)

620 POKE PO,2
622 POKE PO+1,0:POKEPO+2,0
623 POKE PO+3,BW+1
624 POKE PO+4,BH

630 VTR VP,PO+5,FL

640 WF$="@"+LEFT$(PF$,LEN(PF$)-4)+".VMS2"
645 PRINT "SAVING "WF$
650 BSAVE (WF$),B0,U(DD),P(DB) TO P(PO+5+FL)
...
782 DEF FN LB(ZZ)=DEC(RIGHT$(HEX$(ZZ),2))
784 DEF FN HB(ZZ)=DEC(LEFT$(HEX$(ZZ),2))
