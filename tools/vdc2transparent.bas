...
#270 NEXT
230 PF$="1.VMS2":GOSUB290
231 PF$="2.VMS2":GOSUB290
232 PF$="6.VMS2":GOSUB290
233 PF$="7.VMS2":GOSUB290
234 PF$="13.VMS2":GOSUB290
235 PF$="14.VMS2":GOSUB290
280 BANK15:END
...
390 BW=PEEK(DB+9):BH=PEEK(DB+10):ID=DB+11:PRINT"IMAGE DATA STARTS AT "ID
395 PRINT " WIDTH:"BW", HEIGHT:"BH

#   RL:REPEAT LINE COUNT. 
400 RO=0:WO=0:RC=0:CC=0:RT=AP:VT=0:RR=0:RL=0
...
500   RC=RC+1:RO=RO+1:RL=RL+1

510   WO=WO+1:ID=ID+1
515  NEXT

520  IF RC>0 THEN GOSUB 595
...
560 VTR AP,PO,IL
565 BSAVE ("@"+PF$+".VMS1"),B0,P(SP)TOP(PO+IL)
566 ATTR 0
...
595 IF RL=BW THEN RR=RR+1:PRINT RR:RETURN
596 IF RR>0 THEN GOSUB 1200
597 IF RC>0 THEN GOSUB 1000
599 RETURN
...
# WRITE VMC DATA
# RC:READ COUNT (NUMBER OF BYTES READY FOR VMC)
# RS:RTV-SOURCE
# RT:RTV-TARGET
# VC:VRAM COUNT (NUMBER OF BYTES TO BE COPIED TO VRAM)
# RO:READ-OFFSET. LIKE ID, BUT RELATIVE TO BEGIN OF IMAGE DATA
# ID:IMAGE-DATA ADDRESS. CURRENT BYTE TO BE WORKED ON
...
#    WRITE VMC-2 DATA (FULL, REPEATED, NON-TRANSPARENT LINES)
1200 BANK 0
1205 VC=BW:VT=WO-RC
1210 POKE PO,2:PO=PO+1
1220 POKE PO,FNLB(VT):PO=PO+1
1230 POKE PO,FNHB(VT):PO=PO+1
1240 POKE PO,FNLB(VC):PO=PO+1
1250 POKE PO,FNLB(RR):PO=PO+1
1260 CC=CC+1
1270 RC=RC-RR*BW
1280 RS=ID-RC:RT=RT+VC
1290 FOR Z=1 TO RR
1292  RTV RS,RT,VC:PRINT "RTV "RS","RT","VC
1294  RS=RS+VC:RT=RT+VC
1296 NEXT

1298 VC=BW*RR:RR=0:PRINT
